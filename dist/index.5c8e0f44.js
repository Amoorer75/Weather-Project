...
function getSearchTerm() {
    ...
    if (isNaN(weatherCheck) === true) // console.log(weatherCheck)
    searchString = `q=${weatherCheck}`;
    else // console.log(weatherCheck)
    searchString = `zip=${weatherCheck},us`;
    findWeather(searchString);
}
function findWeather(searchTerm) {
    axios.get(`${BASE_URL}${searchTerm}&units=imperial&appid=${API_KEY}`).then((response)=>{
...
function tempChange(temp, minTemp, maxTemp) {
    if (isNaN(temp) === false && temp < 40) temp.innerHTML.style.text = "blue";
    else if (isNaN(temp) === false && temp > 50) temp.innerHTML.style.text = "red";
}
